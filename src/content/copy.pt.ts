...
export const copyPt: Copy = {
  ...
  projects: {
    ...
    list: [
      ...
      {
        id: "reddy", name: "Reddy", tag: "IA · Atendimento automatizado", trophy: "// MVP EM PRODUÇÃO · N8N + IA + WHATSAPP",
        desc: "Agente de IA que assume o suporte técnico via WhatsApp: fluxo de diagnóstico guiado (cadastro → conexão → dispositivo), detecção automática de idioma por DDI e memória de conversa, reduzindo a dependência de atendimento humano em triagens recorrentes.",
        problem: "Suporte técnico via WhatsApp dependia 100% de atendimento humano para triagem de problemas recorrentes (conexão, cadastro, dispositivos físicos), sem histórico de conversa nem padronização de resposta.",
        solution: "Agente de IA integrado ao Chatwoot via Twilio, atendendo pelo número oficial da empresa. Orquestração via n8n, com detecção automática de idioma do cliente (pelo DDI, com fallback para PT-BR) e fluxo de diagnóstico guiado, refinado a partir de feedback real de uso.",
        tech: ["n8n", "Twilio", "Chatwoot", "GPT-4o", "WhatsApp Business API"],
        role: "Full-stack + IA · time interno",
        results: [
          { label: "Status", value: "MVP em produção" },
          { label: "Canal", value: "WhatsApp Business", context: "suporte técnico" },
        ],
      },
      {
        id: "cpm", name: "CPM", tag: "Sistema Web Custom · CRM comercial", trophy: "// MVP EM DESENVOLVIMENTO · REACT + NESTJS",
        desc: "CRM comercial construído do zero: pipeline de vendas, funil, analytics e detecção automática de oportunidades sem follow-up (\"Anti-Limbo\"), com ambientes de produção e staging separados e deploy controlado por branch.",
        problem: "Equipe comercial sem uma ferramenta própria de gestão de pipeline, funil de vendas e leads — processo disperso entre planilhas e ferramentas genéricas, sem visibilidade de oportunidades esquecidas no funil.",
        solution: "CRM comercial full-stack construído do zero: pipeline de vendas, funil, analytics e sistema \"Anti-Limbo\" (detecção de oportunidades paradas sem follow-up). Ambientes de produção e staging separados, com backups automáticos diários e fluxo de deploy via branch (staging → aprovação → produção).",
        tech: ["React", "Vite", "NestJS", "Prisma", "PostgreSQL", "GitHub Actions"],
        role: "Full-stack · arquitetura própria",
        results: [
          { label: "Ambientes", value: "Prod + Staging", context: "deploy controlado por branch" },
          { label: "Backup", value: "Diário", context: "retenção de 7 dias" },
        ],
      },
      {
        id: "dashboard-financeiro", name: "Dashboard Financeiro", tag: "BI · Business Intelligence", trophy: "// ENTREGUE E HOMOLOGADO · SQL + BI",
        desc: "Modelagem completa do DRE industrial (Receita Bruta, CMV, Margem Bruta, EBITDA, Lucro Líquido) a partir dos lançamentos financeiros do ERP, com painel executivo de KPIs e validação de paridade 1:1 contra os relatórios nativos.",
        problem: "Time financeiro sem visão consolidada de DRE industrial a partir dos dados brutos lançados no ERP, dependendo de conferência manual linha a linha.",
        solution: "Modelagem completa do DRE industrial (modelo PIER) a partir da tabela de lançamentos financeiros do ERP, com views de resumo, painel executivo de KPIs e gráficos de evolução/composição. Validação de paridade com dados reais antes da aprovação, e documentação técnica completa entregue para manutenção futura.",
        tech: ["SQL", "Modelagem de dados (DRE/PIER)", "Figma", "BI"],
        role: "BI · modelagem de dados",
        results: [
          { label: "Status", value: "Entregue e homologado" },
          { label: "Paridade", value: "1:1", context: "validado contra relatórios nativos do ERP" },
        ],
      },
      {
